...
```{r echo=FALSE}
mis_num <- nrow(D[!complete.cases(D),])
```
The total number of missing values in the dataset is **`r mis_num`**.
...
```{r CompareHistograms, echo=FALSE}
h1 <- hist(ds$number_steps, plot=FALSE)
h2 <- hist(df$number_steps, plot=FALSE)
hh <- data.frame(mids=h1$mids, remove_missing=h1$counts, imputing_missing=h2$counts)
hh <- melt(hh, id.vars="mids")
ggplot(data=hh, aes(mids, value, fill=variable)) + geom_bar(stat="identity", position=position_dodge()) + 
